...
for cluster in ablation_clusters:
    ...
    if data_config["dataset"] == "Cora":
        ...
        if model_config["clustering_type"] == "hsg":
            hiearchical_data = hierarchical_clustering(train_dataset[0], num_clusters_per_level=cluster, num_levels=len(cluster), cora=True)
        elif model_config["clustering_type"] == "unet":
            hiearchical_data = hierarchical_reverse_clustering(train_dataset[0], num_clusters_per_level=cluster, num_levels=len(cluster), unet=False, cora=True)
        elif model_config["clustering_type"] == "unet_with_skip":
            hiearchical_data = hierarchical_reverse_clustering(train_dataset[0], num_clusters_per_level=cluster, num_levels=len(cluster), unet=True, cora=True)
        ...
        train_dataset = load_processed_lrgb_dataset(name=data_config["dataset"], clustering_type=model_config["clustering_type"], split="train", root="./data/LRGB", overwrite=True, clusters=cluster)
        # Create a DataLoader for batching
        train_loader = DataLoader(train_dataset, batch_size=train_config["batch_size"], shuffle=True)
        # repeat for validation dataset
        val_dataset = load_processed_lrgb_dataset(name=data_config["dataset"], clustering_type=model_config["clustering_type"], split="val", root="./data/LRGB", overwrite=True, clusters=cluster)
    ...
    if __name__ == "__main__":
        if data_config["dataset"] == "Cora":
            final_F1_scores.append(train_cora(train_dataset))
        else:
            final_F1_scores.append(train())
